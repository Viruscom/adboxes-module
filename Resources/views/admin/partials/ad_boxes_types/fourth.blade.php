<div class="row">
    <div class="col-xs-12">
        <h3>@lang('adboxes::admin.adboxes.index'): @lang('adboxes::admin.ad_boxes_type_4')</h3>
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                <?php $i = 1; ?>
                @forelse ($adBoxesAdminAll[4] as $adBox)
                    <tr class="t-row row-{{$adBox->id}}" data-toggle="popover" data-content='<img class="thumbnail img-responsive" src="{{ $adBox->getFileUrl() }}"/>'>
                        ...
                        <td><label class="label btn-light-blue">@lang('adboxes::admin.ad_boxes_type_4')</label></td>
                        <td>{{ $adBox->title }}</td>
                        <td class="pull-right">
                            @include('admin.partials.index.action_buttons', ['mainRoute' => Request::segment(2), 'models' => $adBoxesAdminAll[4], 'model' => $adBox, 'showInPublicModal' => false])
                        </td>
                    </tr>
...
<div class="row">
    <div class="col-md-12">
        <a href="{{ route('admin.ad-boxes.edit-button', ['adBoxType' => 4]) }}" class="btn btn-light-green">{!! (isset($adBoxButtons[3]) && $adBoxButtons[3]->title !='') ? $adBoxButtons[3]->title : trans('adboxes::admin.default_button_name_4')  !!}</a>
    </div>
</div>
